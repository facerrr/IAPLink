...
EasyUIPage_t ZSeries, XSeries, DownloadPage;
...
void MenuInit(void)
{
    ...
    EasyUIAddPage(&DownloadPage, PAGE_CUSTOM);
    ...
    for(int i = 0; i < 8; i++) {
        ...
        if(IAP_ReadFileInfo(fileInfoStart, u8FileNames[i], 32)) {
            EasyUIAddItem(pages[i], iapItems[i], u8FileNames[i], ITEM_MESSAGE, "Downloading...", EasyUIEventIAPFinished);
        } else {
            // 文件不存在或无效，使用默认名称
            EasyUIAddItem(pages[i], iapItems[i], "File Name Invalid", ITEM_MESSAGE, "Downloading...", EasyUIEventIAPFinished);
        }
    }
