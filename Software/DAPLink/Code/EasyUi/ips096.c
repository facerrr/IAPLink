...
uint16_t IPS096_penColor = IPS096_DEFAULT_PENCOLOR;
uint16_t IPS096_backgroundColor = IPS096_DEFAULT_BGCOLOR;
...
static uint8_t spi_tx_buffer[25600];


#define ips096_write_8bit_data(data)    (spi_write_8bit(data))
#define ips096_write_16bit_data(data)   (spi_write_16bit(data))
...
void IPS096_SetRegion (const uint16_t x1, const uint16_t y1, const uint16_t x2, const uint16_t y2)
{
    if(ips096_display_dir == IPS096_PORTAIT){
        ips096_write_index(0x2a);
        ips096_write_16bit_data(x1 + 24);
        ips096_write_16bit_data(x2 + 24);
        ips096_write_index(0x2b);
        ips096_write_16bit_data(y1);
        ips096_write_16bit_data(y2);
        ips096_write_index(0x2c);
...
void IPS096_SendBuffer(void)
{
    IPS096_CS(0);
    IPS096_SetRegion(0, 0, 159, 79);
    #if DMA_ENABLE
    uint32_t index = 0;
    uint16_t *src = (uint16_t *)IPS096_buffer;
    for (int i = 0; i < 80 * 160; i++) {
        spi_tx_buffer[index++] = (uint8_t)(*src >> 8); // 高位
        spi_tx_buffer[index++] = (uint8_t)(*src & 0xFF); // 低位
        src++;
    }
    spi_send_by_dma(spi_tx_buffer, index);
    #else
    for (int i = 0; i < 80; i++)
    {
        for (int j = 0; j < 160; j++)
...
/*!
 * @brief   IPS096 画点
 * @param   x           坐标x方向的位置 参数范围 [0, ips096_x_max-1]
 * @param   y           坐标y方向的位置 参数范围 [0, ips096_y_max-1]
 * @param   color       颜色格式 RGB565 可以使用 zf_common_font.h 中 rgb565_color_enum 枚举值或者自行写入
 * @return  None
 * @note    ips096_draw_point(0, 0, RGB565_RED);            // 坐标 0,0 画一个红色的点
 */
void IPS096_DrawPoint(int16_t x, int16_t y, const uint16_t color)
{
 ...
 * @param   color       颜色格式 RGB565 可以使用 zf_common_font.h 中 rgb565_color_enum 枚举值或者自行写入
 * @return  None
 * @note    ips096_draw_line(0, 0, 10, 10, RGB565_RED);     // 坐标 0,0 到 10,10 画一条红色线段
 */
void IPS096_DrawLine(int16_t x_start, int16_t y_start, int16_t x_end, int16_t y_end, const uint16_t color)
{
 ...
 * @param   dat         需要显示的字符
 * @return  None
 * @note    ips096_show_char(0, 0, 'A');                     // 坐标 0,0 显示字符 A
 */
void IPS096_ShowChar(int16_t x, int16_t y, const char dat)
{
    uint8_t i, j;
 ...
 * @param   dat         需要显示的字符串
 * @return  None
 * @note    ips096_show_string(0, 0, "seekfree");
 */
void IPS096_ShowStr(int16_t x, int16_t y, const char dat[])
{
    uint16_t j = 0;
    while (dat[j] != '\0')
    {
...
/*!
 * @brief   IPS096 显示32位有符号 (去除整数部分无效的0)
 * @param   x           坐标x方向的位置 参数范围 [0, ips096_x_max-1]
 * @param   y           坐标y方向的位置 参数范围 [0, ips096_y_max-1]
 * @param   dat         需要显示的变量 数据类型 int32
 * @param   num         需要显示的位数 最高10位  不包含正负号
 * @return  None
 * @example ips096_show_int(0, 0, x, 3); 
 * @note    负数会显示一个 '-'号   正数显示一个空格
 */
void IPS096_ShowInt(int16_t x, int16_t y, const int32_t dat, uint8_t num)
{
//    zf_assert(num > 0);
//    zf_assert(num <= 10);
...
/*!
 * @brief   IPS096 显示32位无符号 (去除整数部分无效的0)
 * @param   x           坐标x方向的位置 参数范围 [0, ips096_x_max-1]
 * @param   y           坐标y方向的位置 参数范围 [0, ips096_y_max-1]
 * @param   dat         需要显示的变量 数据类型 uint32
 * @param   num         需要显示的位数 最高10位  不包含正负号
 * @return  None
 * @example ips096_show_uint(0, 0, x, 3); 
 * @note    负数会显示一个 '-'号   正数显示一个空格
 */
void IPS096_ShowUint(int16_t x, int16_t y, const uint32_t dat, uint8_t num)
{
//    zf_assert(num > 0);
//    zf_assert(num <= 10);
 ...
 * @return  None
 * @example ips096_show_float(0, 0, x, 2, 3); 
 * @note    特别注意当发现小数部分显示的值与你写入的值不一样的时候，
 *          可能是由于浮点数精度丢失问题导致的，这并不是显示函数的问题，
 *          有关问题的详情，请自行百度学习   浮点数精度丢失问题。
 *          负数会显示一个 '-'号   正数显示一个空格
 */
void IPS096_ShowFloat(int16_t x, int16_t y, const float dat, uint8_t num, uint8_t pointnum)
{
//    zf_assert(num > 0);
//    zf_assert(num <= 8);
 ...
 * @return  None
 * @note    ips096_draw_circle(80, 40, 20, RGB565_RED, CIRCLE_DRAW_ALL);     // 画一个红色的圆
 */
...
/*!
 * @brief   IPS096 显示彩色位图 (RGB565格式)
 * @param   x           坐标x方向的位置
 * @param   y           坐标y方向的位置
 * @param   width       图像宽度
 * @param   height      图像高度
 * @param   pic         RGB565图像数据指针 (每像素2字节)
 * @return  None
 */
void IPS096_ShowBMP_Color(int16_t x, int16_t y, uint16_t width, uint16_t height, const uint8_t *pic)
{
    ...
    for (y_pos = 0; y_pos < height; y_pos++)
    {
        ...
        {
            uint32_t index = (y_pos * width + x_pos) * 2;
            color = pic[index] | (pic[index + 1] << 8);
            color = ~color;
            IPS096_DrawPoint(x + x_pos, y + y_pos, color);
        }
    }
}
...
void IPS096_SetColor (const uint16_t pen, const uint16_t bgcolor)
{
    IPS096_penColor = pen;
...
void IPS096_Init(void){
    ...
    gpio_init_struct.gpio_pins = IPS096_SCK_PIN;
    gpio_init_struct.gpio_pull = GPIO_PULL_NONE;
    gpio_init(IPS096_SCK_PORT, &gpio_init_struct);

    gpio_init_struct.gpio_mode = GPIO_MODE_MUX;
    gpio_init_struct.gpio_pins = IPS096_SDA_PIN;
    gpio_init(IPS096_SDA_PORT, &gpio_init_struct);

    gpio_init_struct.gpio_mode = GPIO_MODE_OUTPUT;
    gpio_init_struct.gpio_pins = IPS096_CS_PIN;
    gpio_init(IPS096_CS_PORT, &gpio_init_struct);
    gpio_bits_set(IPS096_CS_PORT, IPS096_CS_PIN);

    gpio_init_struct.gpio_pins = IPS096_DC_PIN;
    gpio_init(IPS096_DC_PORT, &gpio_init_struct);
    gpio_bits_reset(IPS096_DC_PORT, IPS096_DC_PIN);

    gpio_init_struct.gpio_pins = IPS096_RST_PIN;
    gpio_init(IPS096_RST_PORT, &gpio_init_struct);
    gpio_bits_reset(IPS096_RST_PORT, IPS096_RST_PIN);

    gpio_init_struct.gpio_pins = IPS096_BLK_PIN;
    gpio_init(IPS096_BLK_PORT, &gpio_init_struct);
    gpio_bits_set(IPS096_BLK_PORT, IPS096_BLK_PIN);
