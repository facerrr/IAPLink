...
#define SEND_32BIT_ONCE_FAST()                            \
    if (val & 1)                                          \
    {                                                     \
        PIN_CLK_GPIO->scr = PIN_DIO_O | (PIN_CLK << 16U); \
    }                                                     \
    else                                                  \
    {                                                     \
        PIN_CLK_GPIO->scr = (PIN_DIO_O | PIN_CLK) << 16U; \
...
#define SW_CLOCK_CYCLE_FAST()   PIN_SWCLK_CLR();PIN_DELAY_FAST();  PIN_SWCLK_SET(); PIN_DELAY_FAST();
#define SW_READ_BIT_FAST(bit)   PIN_SWCLK_CLR(); PIN_DELAY_FAST();  bit = PIN_SWDIO_IN(); PIN_SWCLK_SET(); PIN_DELAY_FAST();
