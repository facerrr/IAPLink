...
uint8_t u8FileDownLoadFlag = 0;

void AppToBoot(void){
    uint32_t iapFlag;
    __disable_irq();
    if(u8FileDownLoadFlag){
        u8FileDownLoadFlag = 0;
        iapFlag = APP_UPGRADE;
        Flashx_EraseSector(FLASH_INFO_ADDR_ST);
        Flashx_WriteBytes(FLASH_INFO_ADDR_ST, (uint8_t*)(&iapFlag), 4);
    }else{
        iapFlag = APP_FLAG2;
        Flashx_EraseSector(FLASH_INFO_ADDR_ST);
        Flashx_WriteBytes(FLASH_INFO_ADDR_ST + 4, (uint8_t*)(&iapFlag), 4);
    }
    __enable_irq();
    NVIC_SystemReset();
...
error_status IAP_ReadFileInfo(uint32_t fileAddr, char *outFileName, uint16_t maxLen){
    ...
    Flashx_ReadBytes(fileAddr, (uint8_t*)&fileInfo, sizeof(FileInfoStruct));

    if(fileInfo.FileNameLen == 0 || fileInfo.FileNameLen >= maxLen) {
        strncpy(outFileName, "NO FILE", maxLen - 1);
        outFileName[maxLen - 1] = '\0';
        return false;
    }
    Flashx_ReadBytes(fileAddr + sizeof(FileInfoStruct), (uint8_t*)outFileName, fileInfo.FileNameLen);
    outFileName[fileInfo.FileNameLen] = '\0';  // 确保字符串结束
    return true;
}